...
(*   "prune과 미래 연산의 교환성". AddTag의 cascade는 코드 그대로 묘비에서 *)
(*   절단된다(prune이 childIndex 엣지를 지움 — lineage.ts pruneSession-    *)
(*   Lineage의 perSession.delete). 절단된 세계와 "prune이 없었다면 관통    *)
(*   전파됐을" 세계의 비교는 두-세계 lockstep 모델의 몫이다.               *)
(*                                                                         *)
