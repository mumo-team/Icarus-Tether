...
function resolveParents(
  ...
  //  - ★ 회귀 가드(발동조건 확대 금지): floor 스킵은 흐름분리 정밀도의 대가다.
  //    디코이(U 노드 매칭)가 스킵을 유발하는 세탁 FN은 floor로 못 막으며
  //    (스킵 조건을 없애는 확대안 실측: 과차단 9.3%→18.5%로 session 모드 회귀,
  //    미탐 개선 0 — RB01-05 흐름분리가 전부 FP로 뒤집힘), 값-내용을 직접 보는
  //    출력-스캔(TIER3, output-scan.ts)이 그 벡터(RS11/12/13)를 담당한다.
