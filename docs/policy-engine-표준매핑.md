...
| 표준 위협 | 우리 대응 | 어떻게 |
|---|---|---|
| **LLM01: Prompt Injection** | ✅ 핵심 대응 | 비신뢰 소스(웹·이슈 등)에 숨은 악성 지시가 AI를 조작해도, 그 결과 데이터 흐름을 값 단위로 추적해 **유출 시점에 결정론 차단**. 인젝션 자체를 "탐지"하려 하지 않고(우회 가능), 인젝션이 노리는 **행동(민감+비신뢰의 외부 전송)을 차단**. |
...
| **거버넌스·관측성** | ✅ append-only 감사 로그, 판정별 explanation, 섀도 로그로 투명성 제공. (감사 로그 암호학적 서명·영속화는 향후 보강 항목.) |
...
⬜ 향후·범위밖: LLM08(벡터), ASI07(멀티에이전트), 장기메모리 포이즌
   ...
   + 향후 보강: 감사 로그 암호학적 서명·영속화 (현재 append-only 인메모리), 
