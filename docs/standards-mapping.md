...
| **LLM05: Improper Output Handling** | ✅ 대응 | LLM 출력을 "비신뢰 데이터"로 취급하는 원칙과 정합(도구 결과를 UNTRUSTED_ORIGIN으로 태그). 나가는 값의 *내용*을 직접 보는 **출력-스캔**(output-scan.ts)이 인코딩 세탁을 차단한다: verbatim 포함·청크 재조립·base64(표준/URL-safe)·percent·hex·재포맷 정규화 매칭. 엔트로피는 의도적으로 쓰지 않는다(SHA·UUID·JWT 과차단 방지). 잔여 한계는 §6. |
| **LLM06: Excessive Agency** | ✅ 대응 | 유출 행동(민감+비신뢰가 외부 싱크로 수렴)에 HITL(사람 승인) 게이트 + 결정론 차단. 되돌리기 어려운 **파괴적 행동은 유출과 직교인 전용 게이트**로 별도 처리 — "삭제 자체"가 아니라 "비신뢰가 유발한 삭제"만 HITL을 요구하고, 사용자가 직접 지시한 삭제는 구조적으로 통과한다(의미론을 TaintDestructiveHITL.tla로 선행 확정, 위반 0). |
...
| **ASI03: Identity & Privilege Abuse** | 🔶 부분 | 세션 단위 권한 경계. HITL로 고위험 행동 승인(제어 채널은 프로세스별 토큰 인증). (교차 세션 방어는 알려진 한계.) |
...
```
✅ 직접 대응: LLM01, LLM02, LLM05, LLM06 / ASI01, ASI02
🔶 부분 대응: ASI03, ASI06
⬜ 향후·범위밖: LLM08(벡터), ASI07(멀티에이전트), 장기메모리 포이즌
   + 알려진 한계(원리적): 제어-흐름 공격(도구 description 인젝션 — 데이터흐름 모델의 사각지대),
     의역 유출(원본 바이트가 안 남음), 다층 인코딩 조합(예: hex(base64(s))),
     인코딩 전 변형(압축·암호화 후 인코딩), 12자 미만 짧은 민감값, 교차 세션
   + 향후 보강: 감사 로그 HMAC 키 기반 서명 (현재 해시체인 무결성 검증까지 구현 —
     변조 탐지 O, 위조 방지는 향후)
  + 향후 보강: 대시보드 제어 채널(승인·정화) 인증 — 프로세스마다 발급하는 토큰으로
     인증하며, 토큰은 루프백 소켓 밖으로 나가지 않고 OS 임시 디렉터리에 0600으로 둔다.
     같은 계정 권한으로 그 파일을 읽을 수 있는 주체는 승인을 위조할 수 있다 —
     OS 자격증명 저장소 연동은 향후

※ 각 한계의 근거와 "왜 막을 수 있는데 안 막기로 했는지"는 policy-engine/README.md §3에 정리돼 있다.
```
